...
const attendanceController = {
  ...
  recordAttendance: async (req, res, next) => {
    console.log("🚀 출석 기록 프로세스 시작");
    console.log("🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥🔥");
    try {
      const { organizationId, activityId } = req.params;
      const { instanceData, attendances, imageInfo } = req.body;

      // 임시 사용자 ID 설정 (나중에 인증 시스템으로 대체해야 함)
      const currentUserId = 1; // 임시 사용자 ID

      console.log("📦 받은 데이터 전체:");
      ...
      // imageInfo 로깅 추가
      console.log("🖼️ imageInfo 상세 정보:");
      console.log(JSON.stringify(imageInfo, null, 2));
      console.log("imageInfo의 타입:", typeof imageInfo);
      if (typeof imageInfo === "object" && imageInfo !== null) {
        console.log("imageInfo의 키들:", Object.keys(imageInfo));
        for (const [key, value] of Object.entries(imageInfo)) {
          console.log(`${key}:`, value);
          console.log(`${key}의 타입:`, typeof value);
        }
      } else {
        console.log("imageInfo가 객체가 아니거나 null입니다.");
      }
      ...
      console.log("✅ 활동 확인 완료");

      // 활동 인스턴스 생성
      console.log("📝 활동 인스턴스 생성 중...");
      ...
      console.log("✅ 활동 인스턴스 생성 완료:", activityInstance.id);
      ...
      if (
        ...
        try {
          ...
          if (fileData) {
            ...
            console.log(
              "✅ ActivityInstanceHasFile 모델 생성 완료:",
              JSON.stringify(activityInstanceHasFile, null, 2)
            );
          }

          console.log("✅ 파일 정보 저장 완료");
        } catch (fileError) {
      ...
      // 출석 정보 생성 또는 업데이트
      console.log("📊 출석 정보 처리 중...");
      const updatedAttendances = await Promise.all(
        attendances.map(async (attendance) => {
          console.log("👤 사용자 출석 처리:", attendance.userId);
          console.log("출석 상태:", attendance.status);
          ...
          console.log("찾은 출석 상태:", attendanceStatus.name);
